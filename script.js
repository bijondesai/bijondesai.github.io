...
async function recordVote(guess) {
  ...
  const key = guess.toLowerCase(); // 'boy' or 'girl'
  try {
    // Get current count
...
// --- Stats Page (secret, for you only) ---
...
function makeGuess(choice) {
  ...
  const countdown = setInterval(async () => {
    ...
    if (seconds <= 0) {
      ...
      const othersCount = Math.max(0, sameGuessCount - 1); // subtract current user

      let funMessage = '';
      if (isCorrect) {
        if (othersCount === 0) funMessage = "You're the first one to get it right! 🏆";
        else funMessage = `You and ${othersCount} other${othersCount === 1 ? '' : 's'} nailed it! 🎉`;
      } else {
      ...
      document.getElementById('reveal').innerHTML =
        ...
        '<span style="font-size:0.95rem;color:#666;margin-top:14px;display:block;font-weight:400;">' + funMessage + '</span>';
